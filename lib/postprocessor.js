...
class DocxPostProcessor {
  ...
  constructor(template, data, options, filestore) {
    ...
    let oldRids = [];
    documentXmlFile.data = documentXmlFile.data.replaceAll(alldrawings, (drawing) => {
      ...
      if (newfile) {
        ...
        documentXmlRelsFile.data = documentXmlRelsFile.data.replaceAll(allrels, (relationship) => {
          const [,id] = /Id="(.*?)"/.exec(relationship) || [];
          if (id != relationshipId) return relationship;
          if (oldRids.includes(id)) {
            let newRid = `rId1_${this.getRandomRid()}`;
            let newRelationship = relationship.replace(`Id="${id}"`, `Id="${newRid}"`);
            newRelationship = newRelationship.replace(/Target=".*?"/g, `Target="/${imgFile}"`);
            drawing = drawing.replace(`r:embed="${id}"`, `r:embed="${newRid}"`);
